...
def repo_file(repo, *path, mkdir=False):
    """
    Same as repo_path, but create dirname(*path) if absent and if makedir. 
...
def repo_dir(repo, *path, mkdir=False):
    """Same as repo_path, but mkdir *path if absent and if mkdir."""
...
def object_hash(fd, fmt, repo=None):
    """Hash object, writing it to repo of provided."""
...
def ls_tree(repo, ref, recursive=None, prefix=""):
    ...
    for item in obj.items:
        ...
        match type: # Determine the type.
            case b"04": type = "tree" # @FIXME on tutorial "04" but I found out " 4" really work
        ...
        if not (recursive and type == "tree"): # This is a leaf
            print("{0} {1} {2}\t{3}".format(
                ...
                type, # Git's ls-tree displays the type of the object pointed to. We can do that too :)
                item.sha,
...
def ref_resolve(repo, ref):
    ...
    # we're looking for HEAD on a new repository with no commits. In that case, .git/HEAD
    # points to "ref: refs/heads/main", but .git/refs/heads/main doesn't exist yet 
...
def tag_create(repo, name, ref, create_tag_object=False):
    ...
    if create_tag_object:
        ...
        # Notice you ca fix this after commit, read on!
...
def index_read(repo):
    ...
    for i in range(0, count):
        ...
        # Read creation time, as nanoseconds after taht timestamps, for extra precision
        ...
        # Group ID
        gid = int.from_bytes(content[idx+32: idx+26], "big")
        ...
        # Parse flags
        flag_assume_valid = (flags & (0b1 << 15)) != 0
        flag_extended = (flags & (0b1 << 14)) != 0
        assert not flag_extended
        flag_stage = flags & (0b1 << 13 | 0b1 << 12)

        # Length of the name. This is stored on 12 bits, some max value is 0xFFF, 4095. Since names can occasionally go beyond that length,
        # git treats 0xFFF as meaning at least 0xFFF, and looks for the final 0x00 to find the end of the name -- at a small, and probably
        # very rare, performance cost.
        ...
        if name_length < 0xFFF:
            ...
            # This probably wasn't tested enough. It works with a path of exactly 0xFFF bytes. Any extra bytes broke
        ...
        # Data is padded on multiples of 8 bytes for pointer alignment,
        # so we skip as many bytes as we need for the next read to start at the right position
        idx = 8 * ceil(idx / 8)
...
def cmd_ls_files(args):
    ...
    for e in index.entries:
        ...
        if args.verbose:
            ...
            print(" flags: stage={}, assume_valed={}".format(e.flag_stage, e.flag_assume_valid))
...
def gitignore_parse(lines):
    ...
    for line in lines:
        parsed = gitignore_parse_rule(line)
        if parsed:
            ret.append(parsed)
...
def check_ignore_compare(rules, path):
    result = None
...
def check_ignore_scoped(rules, path):
    ...
    while True:
        if parent in rules:
            result = check_ignore_compare(rules[parent], path)
            if result != None:
                return result
...
def check_ignore_absolute(rules, path):
    ...
    for ruleset in rules:
        result = check_ignore_compare(ruleset, path)
        if result != None:
            return result
...
def tree_to_dict(repo, ref, prefix=""):
    ...
    for leaf in tree.items:
        ...
        # we could just open it as a file and read teh first few bytes)
        is_subtree = leaf.mode.startswith(b"04") or leaf.mode.startswith(b" 4")# @FIXME on tutorial "04" but I found out " 4" really work

        # Depending on the type, we either store the path(if it's a blob, so a regular file),
...
def cmd_status_head_index(repo, index):
    ...
    # Keys still in HEAD are files that we haven't met in the index, and thus have been deleted.
...
def index_write(repo, index: GitIndex):
    with open(repo_file(repo, "index"), "wb") as f:
        ...
        for e in index.entries:
            ...
            # We merge back three pieces of data (two flags and the length of the name) on the same two bytes.
...
def rm(repo: GitRepository, paths, delete=True, skip_missing=False):
...
def add(repo:GitRepository, paths, delete=True, skip_missing=False):
    ...
    for path in paths:
        ...
        # Find and read the index. It was modified by rm. (This isn't optimal, good enough for wyag!)
        # @FIXME, though: we could just move the index through commands instead of reading and writing it over again
        index = index_read(repo)
        ...
        for (abspath, relpath) in clean_paths:
            ...
            entry = GitIndexEntry(ctime=(ctime_s, ctime_ns), mtime=(mtime_s, mtime_ns), dev=stat.st_dev, ino=stat.st_ino,
                                  mode_type=0b1000, mode_perms=0o644, uid=stat.st_uid, gid=stat.st_gid, fsize=stat.st_size,
                                  sha=sha, flag_assume_valid=False, flag_stage=False, name=relpath)
...
def tree_from_index(repo: GitRepository, index: GitIndex):
    ...
    for entry in index.entries:
        ...
        # because even if a directory holds no files it will contain at least a tree.
        key = dirname
        while key != "":
    ...
    # Get keys (= directories) and sort them by length descending. This means that we'll always encounter a given path before its parent,
    # which is all we need, since for each directory D we'll need to modify its parent P to add D's tree.
    sorted_paths = sorted(contents.keys(), key=len, reverse=True)

    # This variable will store the current tree's SHA-1.
    # After we're done iterating over our dict, it will contain the hash for the root tree.
    sha = None

    # We go through the sorted list of path (dict keys)
    for path in sorted_paths:
        ...
        for entry in contents[path]:
            ...
            if isinstance(entry, GitIndexEntry): # Regular entry (a file)
                # We transcode the mode: the entry stores as integer, we need an octal ASCII representation for the tree.
...
def commit_create(repo: GitRepository, tree, parent, author, timestamp: datetime, message) -> str:
...
def cmd_commit(args):
    ...
    commit = commit_create(repo, tree, 
    ...
    if active_branch: # If we're on a branch, we update refs/heads/BRANCH
        with open(repo_file(repo, os.path.join("refs/heads", active_branch)), "w") as fd:
            fd.write(commit + "\n")
    else: # Otherwise, we update HEAD ifself.
